...
public interface UserCourseMapper {
    int getEachLevelRecordRows(@Param("userNo") int userNo, @Param("level") int level); // 로그인한 사용자의 현재 도전 가능한 단계에서 달성한 완주 기록 갯수를 가져온다.
    ...
    void insertUserBadge(@Param("userNo") int userNo); // 로그인한 사용자의 배지가 없다면, 1단계 배지를 부여한다.
